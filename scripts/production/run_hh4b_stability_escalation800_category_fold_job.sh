...
for p in result_paths:
    try:
        ...
        if (
            ...
            and x.get("category_id") == os.environ["CATEGORY_ID_VALUE"]
            and int(x.get("outer_fold", -1)) == int(os.environ["OUTER_FOLD_VALUE"])
            and x.get("repository_head") == os.environ["EXPECTED_HEAD_VALUE"]
...
payload = {
    "schema_version": 1,
    "status": (
        "selection_stability_escalation800_category_fold_job_complete"
        if runner_rc == "0" and valid == 27
        else "selection_stability_escalation800_category_fold_job_failed"
    ),
...
echo "SELECTION_STABILITY_ESCALATION800_CATEGORY_FOLD_START" >> "$RUNNER_LOG"
...
[[ "$REPLICA" =~ ^[0-9]+$ ]]
[[ "$OUTER_FOLD" =~ ^[0-4]$ ]]
[[ "$REPLICA" -ge 200 && "$REPLICA" -le 999 ]]
[[ "$CATEGORY_ID" == "exact3tag" || "$CATEGORY_ID" == "ge4tag" ]]
[[ -f "$PAYLOAD_BASENAME" ]]
[[ -f "$RUNTIME_BASENAME" ]]
...
CATEGORY_ID_VALUE="$CATEGORY_ID" \
EXPECTED_HEAD_VALUE="$EXPECTED_HEAD" \
"$PYTHON" - <<'PY_PROV' >> "$RUNNER_LOG" 2>&1
from pathlib import Path
import json, os

p = json.loads((Path(os.environ["PAYLOAD_VALUE"]) / "execution_provenance.json").read_text())
assert p["status"] == "transfer_package_selection_stability_escalation800_category_payload"
assert p["repository_head"] == os.environ["EXPECTED_HEAD_VALUE"]
assert int(p["bootstrap_replica"]) == int(os.environ["REPLICA_VALUE"])
assert 200 <= int(p["bootstrap_replica"]) <= 999
assert p["transferred_category_id"] == os.environ["CATEGORY_ID_VALUE"]
assert p["escalation_200_999_production_authorized"] is True
assert p["results_may_enter_all1000_stability_aggregation"] is True
assert p["pilot_results_may_enter_stability_aggregation"] is False
assert p["nominal_selection_changed"] is False
assert int(p["validation_payloads_opened"]) == 0
assert int(p["test_payloads_opened"]) == 0
print("TRANSFER_PROVENANCE_ROLE=PASS")
PY_PROV
