#!/usr/bin/env bash

set -euo pipefail

export HH4B_REPO="${HH4B_REPO:-/uscms_data/d3/$USER/repos/hh-bbww-baselines}"
export HH4B_STORE="${HH4B_STORE:-/uscms_data/d3/$USER/hh4b_delphes}"
export DELPHES_DIR="${DELPHES_DIR:-/uscms_data/d3/$USER/software/Delphes}"

if [[ "$#" -gt 1 ]]; then
  echo "Usage: $0 [OUTPUT_TARBALL]"
  exit 2
fi

TARBALL="${1:-$HH4B_STORE/condor_inputs/ttbar_existing_hepmc_frozen_v2_inputs.tar.gz}"

INPUT_DIR=$(dirname "$TARBALL")

CARD="$HH4B_REPO/cards/delphes/delphes_card_CMS_lpc_ak4ak8_run2_frozen_v2.tcl"

WRAPPER="$HH4B_REPO/scripts/production/run_ttbar_existing_hepmc_bundle.sh"

EXPECTED_CARD_SHA256="1b2041245162de8360defdc502404e0696496c50773d4aa7f043798651a9517c"

mkdir -p "$INPUT_DIR"

test -x "$WRAPPER"

CARD_SHA256=$(
  sha256sum "$CARD" |
  awk '{print $1}'
)

test "$CARD_SHA256" = "$EXPECTED_CARD_SHA256"

STAGE=$(
  mktemp -d "$INPUT_DIR/ttbar_existing_hepmc_payload.XXXXXX"
)

cleanup() {
  rm -rf "$STAGE"
}

trap cleanup EXIT

PAYLOAD="$STAGE/payload"

mkdir -p \
  "$PAYLOAD/repo/scripts/delphes" \
  "$PAYLOAD/repo/cards/delphes" \
  "$PAYLOAD/Delphes"

cp \
  "$HH4B_REPO/scripts/delphes/make_delphes_event_summary.py" \
  "$HH4B_REPO/scripts/delphes/reconstruct_hh4b_candidates_v2.py" \
  "$HH4B_REPO/scripts/delphes/write_parquet_from_pickle.py" \
  "$PAYLOAD/repo/scripts/delphes/"

cp \
  "$CARD" \
  "$PAYLOAD/repo/cards/delphes/"

for FILE in \
  DelphesHepMC3 \
  libDelphes.so \
  ClassesDict_rdict.pcm \
  ExRootAnalysisDict_rdict.pcm \
  ModulesDict_rdict.pcm \
  ModulesFastJetDict_rdict.pcm \
  DelphesEnv.sh
do
  test -e "$DELPHES_DIR/$FILE"
  cp "$DELPHES_DIR/$FILE" "$PAYLOAD/Delphes/"
done

GIT_HEAD=$(
  git -C "$HH4B_REPO" rev-parse HEAD
)

WRAPPER_SHA256=$(
  sha256sum "$WRAPPER" |
  awk '{print $1}'
)

EVENT_SCRIPT_SHA256=$(
  sha256sum "$HH4B_REPO/scripts/delphes/make_delphes_event_summary.py" |
  awk '{print $1}'
)

CANDIDATE_SCRIPT_SHA256=$(
  sha256sum "$HH4B_REPO/scripts/delphes/reconstruct_hh4b_candidates_v2.py" |
  awk '{print $1}'
)

PARQUET_WRITER_SHA256=$(
  sha256sum "$HH4B_REPO/scripts/delphes/write_parquet_from_pickle.py" |
  awk '{print $1}'
)

cat > "$PAYLOAD/manifest.txt" <<EOF
sample=inclusive_ttbar_existing_hepmc
operation=existing_hepmc_to_frozen_v2_delphes
created_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')
git_head=$GIT_HEAD
card_sha256=$CARD_SHA256
worker_wrapper_sha256=$WRAPPER_SHA256
event_summary_script_sha256=$EVENT_SCRIPT_SHA256
candidate_script_sha256=$CANDIDATE_SCRIPT_SHA256
parquet_writer_sha256=$PARQUET_WRITER_SHA256
EOF

TMP="$TARBALL.tmp"

tar \
  -C "$STAGE" \
  -czf "$TMP" \
  payload

mv "$TMP" "$TARBALL"

echo "Prepared: $TARBALL"
sha256sum "$TARBALL"
du -h "$TARBALL"
