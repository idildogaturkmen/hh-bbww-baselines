# Authoritative results index

The c7v errata package was generated from `b684bc4436252bfd789bcb64c3d2cacfa23a075f`; the c7w categorized-BDT package was generated from `5ef7ba06e10275725078ae2452be2cf50be5c1af`.

- Frozen inputs: c7q--c7u under `/uscms_data/d3/iturkmen/hh4b_delphes/harvest_checkpoints/`.
- Authoritative c7v errata: `pn_c7v_jhep_results_and_figures_20260803_v4`; exact provenance is in `CHECKPOINT_PROVENANCE.md`.
- Common 1000-replica source-member registry provenance and SHA-256: `bootstrap_registry_provenance.tsv`.
- Full inspected asset map: `results_asset_manifest.tsv`.
- Numerical claims: `numerical_claim_registry.tsv`.
- Paper tables: `tables/` and `paper_table_manifest.tsv`.
- Paper figures: `figures/` and `paper_figure_manifest.tsv`.
- Exact plotting data: `source_data/`; LaTeX captions: `captions/`.
- Requested plot support and any unsupported-item reasons: `requested_figure_registry.tsv`.
- Frozen c7t model weights and predictions remain in c7t; their exact identities are in the asset and model tables.
- Reviewed categorized-BDT checkpoint: `pn_c7w_nested_categorized_bdt_20260803_v1`; compact paper assets, exact plotting data, uncertainty tables, and visual-review records are in `categorized_bdt/`.
- SPA-Net remains an explicit method/status placeholder until its separate fail-closed truth-contract audit and sealed checkpoint exist.
