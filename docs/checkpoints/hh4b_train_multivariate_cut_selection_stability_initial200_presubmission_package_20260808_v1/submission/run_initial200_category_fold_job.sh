...
payload = {
    "schema_version": 1,
    "status": (
        "selection_stability_initial200_category_fold_job_complete"
        if runner_rc == "0" and valid == 27
        else "selection_stability_initial200_category_fold_job_failed"
    ),
    ...
    "initial_200_production_authorized": True,
    "results_may_enter_initial_stability_aggregation": True,
...
echo "SELECTION_STABILITY_INITIAL200_CATEGORY_FOLD_START" >> "$RUNNER_LOG"
...
assert p["status"] == "transfer_package_selection_stability_initial200_category_payload"
...
assert p["initial_200_production_authorized"] is True
assert p["results_may_enter_initial_stability_aggregation"] is True
